...
function _toJson(excel, dest, head) {
    for (var i_sheet = 0; i_sheet < excel.worksheets.length; i_sheet++) {
        ...
        if (sheet.data && sheet.data.length > 0) {
            ...
            // 强制加表头 beigin by ocean
            var json_temp = {};
            for (var i_col = 0; i_col < col_type.length; i_col++){
                json_temp[i_col] = col_name[i_col];
            }
            output.push(json_temp)
            // 强制加表头 end
            ...
            // 检查文件是否重复 begin by ocean
            var json_name = sheet.name
            if (json_dict[json_name] == 1){
                console.log("error：",json_name,"has same name,please check")
                throw "excel中的sheet有重复！"
            }
            // json_dict[json_name] = 1
            // 检查文件是否重复 end
