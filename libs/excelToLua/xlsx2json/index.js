...
function exportJson(args) {
    ...
    // 先执行删除操作 begin by ocean
    var old_json = "./json/**/[^~$]*.json";
    var del_path;
    glob(old_json, function(err, files) {
        if (err) {
            console.error("remove old Json error:", err);
            throw err;
        };
        files.forEach(function(element, index, array) {
            del_path = path.join(__dirname, element);
            fs.unlink(del_path)
            console.log('remove old json successfully  -->  ',del_path)
        });
    })
    // 先执行删除操作 end 
